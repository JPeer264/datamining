...
class Process:
    ...
    def init(self):
        ...
        for idx, file in enumerate(top_tracks_files):
            ...
            self.matrix.append([known_songs, new_songs])
        ...
        if self.useSparse:
            ...
            scipy.sparse.save_npz(
                './data_processed/user_top_tracks-user_recent_tracks-short.npz', sparse_matrix)
            np.savetxt('./data_processed/user_top_tracks-user_recent_tracks-short_x_labels.txt', ['known_songs', 'new_songs'], header='labels',
                       delimiter='\t', fmt="%s")
            np.savetxt('./data_processed/user_top_tracks-user_recent_tracks-short_y_labels.txt', user_ids, header='user_ids',
                       delimiter='\t', fmt="%s")
        else:
            ...
            np.savetxt('./data_processed/user_top_tracks-user_recent_tracks-short.txt', normalized_matrix.astype(np.float),
                       delimiter='\t', fmt="%s")
